...
class ThemeCompiler {
    ...
    constructor(adminID: string, themeID: string) {
        this.adminID = adminID;
        this.themeID = themeID;
        this.tempFolder = path.join(__dirname, `../../data/temp/${themeID}`);
    ...
    private async checkVariables() {
        ...
        const content = fs.readFileSync(varsDest, "utf8");
        // Regex to capture variable names after "public static String"
        const regex = /public\s+static\s+String\s+([A-Z0-9_]+)/g;
        ...
        if (variableNames.length > 0) {
            await Theme.findOneAndUpdate(
                ...
                {$set: {variables: variableNames}}, // replace old vars with new
                {new: true}
            );

            this.addLog("Variables found: " + variableNames.join(", "));
            this.addLog("Theme variables saved!");
        } else {
